...
TWIinterface *TWI_MAIN;

	

TWIinterface::TWIinterface(bool type, uint8_t addres, uint8_t len_b)
{
	...
	if (type) 
	{
		this->to_sla = addres;
		this->Len_BYTE = len_b;
		this->TWI_StartSlave(addres);
	}
	else
	{
		this->TWI_StartMaster();
	};
	TWI_MAIN = this;
};


void TWIinterface::SendTWIMaster(uint8_t Adress, uint8_t* DATA,uint8_t length)
{
...
void TWIinterface::TWI_StartMaster(void) //Инициализация TWI интерфейса в режиме мастера
{
...
void TWIinterface::TWI_StartSlave(uint8_t Adress) //Инициализация TWI интерфейса режиме ведомого
{
...
void TWIinterface::TWI_Mast_Transm_mode(void)
{
switch (TWI_ST)
	{
	  ...
	  case TW_MT_DATA_ACK: //подтверждение приема данных 0x28
	  		 	if (TWI_WRITE_DATA_LEN == 0)
				{
					...
					CLR_BIT(TWI_ST_FLAG,DATA_TRANSM);
					//SET_BIT(TWI_ST_FLAG,DATA_TRANSM);
				}
				else
				{
...
void TWIinterface::TWI_Mast_Ress_mode(void)
{

};

void TWIinterface::TWI_Slave_Ress_mode(void)
{
switch (TWI_ST)
	{
	  case   TW_SR_SLA_ACK : //Получен адрес данного устройства и требование чтения 0x60
	    TWI_WRITE_DATA_LEN = Len_BYTE;
	  ...
	  case  TW_SR_DATA_NACK: //Потеря арбиража переходв из ведущего в ведомый   0x88 
		...
		TWCR = (1<<TWINT)|(1<<TWEA)|(1<<TWEN)|(1<<TWIE);
		SET_BIT(TWI_ST_FLAG,DATA_READED);
		break;
	  default:
...
void TWIinterface::TWI_Slave_Transm_mode(void)
{
/*
switch (TWI_ST)
	{
...
{
TWI_MAIN->TWI_ST = TWSR;
#ifdef Master
TWI_MAIN->TWI_Mast_Transm_mode();
#else
TWI_MAIN->TWI_Slave_Ress_mode();
#endif
